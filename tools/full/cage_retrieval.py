...
def train():
    ...
    log_dir = opt.log_dir+'_FULL_cage_retrieval' + '/' + opt.name
    #log_dir = opt.log_dir+'_NEWFULL_cage_retrieval_Table' + '/' + opt.name
    ...
    model_full.load_state_dict(torch.load('Table/model_full_99.pth'))
    #model_full.load_state_dict(torch.load('/home/ubuntu/newdisk/wcw/code/I-RED-main/Complementme_Table_full/model_199.pth'))
    model_full.eval()
    for param in model_full.parameters():
        param.requires_grad = False
    
    
    model_RandD.load_state_dict(torch.load('Table_FULL_cage_deform/chair-12kpt/model_randd_99.pth'),False)
    #model_RandD.load_state_dict(torch.load('/home/ubuntu/newdisk/wcw/code/I-RED-main/Complementme_cage_deform_Table/table-12kpt/model_randd_199.pth'),False)
    #model_RandD.eval()
...
class Retrieval_loss(nn.Module):
    ...
    def cal_R_and_D_loss(self, pred_rd_full, pred_rd_partial,cage_out, min_support_pixels=20, max_bear_error=20.):
    ...
    def get_all_loss_terms_source(self, pred_rd_full=None, pred_rd_partial=None, cage_out=None):
        ...
        loss_rd = self.cal_R_and_D_loss(pred_rd_full, pred_rd_partial,cage_out)
