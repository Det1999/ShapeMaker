...
class Partial_loss(nn.Module):
    def __init__(self, opt):
        super(Partial_loss, self).__init__()
        self.opt = opt
    ...
    def cal_ortho_loss(self, pred_recon):
        loss_ortho = self.criterionOrtho(pred_recon['rot']) * self.opt.partial_weight_ortho
    ...
    def cal_add_noise_loss(self, pred_add_noise=None):
        ...
        if self.opt.partial_add_noise:
            ...
            loss_noised_rot = self.cal_rot_dist(noised_rot_mat, rot_mat,
                                                self.opt.partial_detach_aug_loss) * self.opt.partial_weight_noise_R
            loss_noised_T = self.cal_trans_dist(t_noised_vec, t_vec,
                                                self.opt.partial_detach_aug_loss) * self.opt.partial_weight_noise_T
            if self.opt.partial_add_ortho_aug:
                loss_noised_ortho = self.criterionOrtho(noised_rot_mat) * self.opt.partial_weight_ortho
    ...
    def cal_fps_loss(self, pred_fps=None):
        ...
        if self.opt.partial_fps:
            ...
            loss_fps_rot = self.cal_rot_dist(fps_rot_mat, rot_mat,
                                             self.opt.partial_detach_aug_loss) * self.opt.partial_weight_fps_R
            loss_fps_T = self.cal_trans_dist(t_fps_vec, t_vec,
                                             self.opt.partial_detach_aug_loss) * self.opt.partial_weight_fps_T
            if self.opt.partial_add_ortho_aug:
                loss_fps_ortho = self.criterionOrtho(fps_rot_mat) * self.opt.partial_weight_ortho
    ...
    def cal_knn_loss(self, pred_remove_knn=None):
        ...
        if self.opt.partial_remove_knn:
            ...
            loss_remove_knn_rot = self.cal_rot_dist(remove_knn_rot_mat, rot_mat,
                                                    self.opt.partial_detach_aug_loss) * self.opt.partial_weight_part_R
            loss_remove_knn_T = self.cal_trans_dist(t_remove_knn_vec, t_vec,
                                                    self.opt.partial_detach_aug_loss) * self.opt.partial_weight_part_T
            if self.opt.partial_add_ortho_aug:
                loss_remove_knn_ortho = self.criterionOrtho(remove_knn_rot_mat) * self.opt.partial_weight_ortho
    ...
    def cal_resample_loss(self, pred_resample=None):
        ...
        if self.opt.partial_fps:
            ...
            loss_resample_rot = self.cal_rot_dist(resample_rot_mat, rot_mat,
                                                  self.opt.partial_detach_aug_loss) * self.opt.partial_weight_sample_R
            loss_resample_T = self.cal_trans_dist(t_resample_vec, t_vec,
                                                  self.opt.partial_detach_aug_loss) * self.opt.partial_weight_sample_T
            if self.opt.partial_add_ortho_aug:
                loss_resample_ortho = self.criterionOrtho(resample_rot_mat) * self.opt.partial_weight_ortho
    ...
    def cal_can_rot_loss(self, pred_can_rot=None):
        ...
        if self.opt.partial_apply_can_rot:
            ...
            loss_can_rot_rot = self.cal_rot_dist(can_rot_rot_mat, rot_mat,
                                                 False) * self.opt.partial_weight_can_R
            loss_can_rot_T = self.cal_trans_dist(t_can_rot_vec, t_vec,
                                                 False) * self.opt.partial_weight_can_T
            if self.opt.partial_add_ortho_aug:
                loss_can_rot_ortho = self.criterionOrtho(can_rot_rot_mat) * self.opt.partial_weight_ortho
    ...
    def cal_dual_loss(self, pred_dual):
        if pred_dual is None:
            return 0.0, 0.0, 0.0, 0.0
        if self.opt.partial_dual:
            dual_rot_mat = pred_dual['rot_aug']
            t_dual_vec = pred_dual['t_aug']
            rot_mat = pred_dual['rot']
            t_vec = pred_dual['t']
            pc_in = pred_dual['pc_in']
            pc_dual = pred_dual['pc_out']
            loss_dual_rot = self.cal_rot_dist(dual_rot_mat, rot_mat,
                                              False) * self.opt.partial_weight_dual_R
            loss_dual_T = self.cal_trans_dist(t_dual_vec, t_vec,
                                              False) * self.opt.partial_weight_dual_T
            loss_dual_recon = self.criterionPCRecon(pc_in.permute(0, 2, 1), pc_dual)[0] * \
                              self.opt.partial_weight_recon1 / 2.
            if self.opt.partial_add_ortho_aug:
                loss_dual_ortho = self.criterionOrtho(dual_rot_mat) * self.opt.partial_weight_ortho
                return loss_dual_rot, loss_dual_T, loss_dual_recon, loss_dual_ortho
            else:
                return loss_dual_rot, loss_dual_T, loss_dual_recon, 0.0
        else:
            return 0.0, 0.0, 0.0, 0.0
    ...
    def cal_randrt_loss(self, pred_randRT):
        ...
        if self.opt.partial_randRT:
            ...
            loss_randrt = self.criterionPCRecon(recon_pc_randRT, pc.permute(0, 2, 1))[0] * \
                          self.opt.partial_weight_rand_RT
            return loss_randrt
        else:
            return 0.0

    def get_all_loss_terms(self, pred_recon=None, pred_add_noise=None, pred_fps=None, pred_remove_knn=None,
                           pred_resample=None, pred_randRT=None, pred_can_rot=None, pred_dual=None):
        loss_name_list = {}
        loss_sum = 0.0
        ...
        if self.opt.partial_dual:
            loss_dual_rot, loss_dual_T, loss_dual_recon, loss_dual_ortho = self.cal_dual_loss(pred_dual)
            loss_name_list['dual_rot'] = loss_dual_rot
            loss_name_list['dual_T'] = loss_dual_T
            loss_name_list['dual_recon'] = loss_dual_recon
            loss_sum += (loss_dual_rot + loss_dual_T + loss_dual_recon + loss_dual_ortho)
